...
import { checkRequired } from '../../helpers/validation/validationCommon';
...
class LoginForm extends React.Component {
    ...
    validateField = (fieldName, fieldValue) => {
        ...
        if (fieldName === 'mail'){
            if(!checkRequired(fieldValue)){
                errorMessage = t('forms.emptyFieldError');
            }
        }
    ...
    render() {
        const { t } = this.props;
        const errorsSummary = this.hasErrors() ? t('forms.formHasErrors') : '';
        const fetchError = this.state.fetchError ? `${t('error')}: ${this.state.error.message}` : '';
